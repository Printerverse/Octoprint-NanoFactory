...
var ConnectionLabels = /* @__PURE__ */ ((ConnectionLabels2) => {
  ConnectionLabels2[ConnectionLabels2["syncAllRequest"] = 1] = "syncAllRequest";
  ConnectionLabels2[ConnectionLabels2["syncAllResponse"] = 2] = "syncAllResponse";
  ConnectionLabels2[ConnectionLabels2["profileChanged"] = 3] = "profileChanged";
  ...
  ConnectionLabels2[ConnectionLabels2["jobCancelled"] = 15] = "jobCancelled";
  ConnectionLabels2[ConnectionLabels2["jobFailed"] = 16] = "jobFailed";
  ConnectionLabels2[ConnectionLabels2["jobDeleted"] = 17] = "jobDeleted";
  ConnectionLabels2[ConnectionLabels2["jobPause"] = 25] = "jobPause";
  ConnectionLabels2[ConnectionLabels2["jobResume"] = 26] = "jobResume";
  ConnectionLabels2[ConnectionLabels2["jobPrinting"] = 37] = "jobPrinting";
  ...
  ConnectionLabels2[ConnectionLabels2["actionExecuted"] = 21] = "actionExecuted";
  ConnectionLabels2[ConnectionLabels2["actionDeleted"] = 22] = "actionDeleted";
  ConnectionLabels2[ConnectionLabels2["executeCustomGcode"] = 23] = "executeCustomGcode";
  ConnectionLabels2[ConnectionLabels2["emergencyStop"] = 24] = "emergencyStop";
  ConnectionLabels2[ConnectionLabels2["connectPrinter"] = 27] = "connectPrinter";
  ConnectionLabels2[ConnectionLabels2["disconnectPrinter"] = 28] = "disconnectPrinter";
  ConnectionLabels2[ConnectionLabels2["connectionOptionsChanged"] = 38] = "connectionOptionsChanged";
  ConnectionLabels2[ConnectionLabels2["printerStateChanged"] = 39] = "printerStateChanged";
  ConnectionLabels2[ConnectionLabels2["refreshConnectionOptions"] = 45] = "refreshConnectionOptions";
  ConnectionLabels2[ConnectionLabels2["filamentExtrude"] = 46] = "filamentExtrude";
  ConnectionLabels2[ConnectionLabels2["targetTool"] = 47] = "targetTool";
  ConnectionLabels2[ConnectionLabels2["targetBed"] = 48] = "targetBed";
  ConnectionLabels2[ConnectionLabels2["handshakeRequest"] = 50] = "handshakeRequest";
  ConnectionLabels2[ConnectionLabels2["handshakeResponse"] = 31] = "handshakeResponse";
  ConnectionLabels2[ConnectionLabels2["queuePaused"] = 29] = "queuePaused";
  ConnectionLabels2[ConnectionLabels2["home"] = 59] = "home";
  ConnectionLabels2[ConnectionLabels2["peerPermissionRequest"] = 32] = "peerPermissionRequest";
  ConnectionLabels2[ConnectionLabels2["peerPermissionResponse"] = 33] = "peerPermissionResponse";
  ConnectionLabels2[ConnectionLabels2["peerListModification"] = 36] = "peerListModification";
  ConnectionLabels2[ConnectionLabels2["peerSyncRequest"] = 60] = "peerSyncRequest";
  ConnectionLabels2[ConnectionLabels2["peerSyncResponse"] = 61] = "peerSyncResponse";
...
var OctoPrintEndPoint = /* @__PURE__ */ ((OctoPrintEndPoint2) => {
  OctoPrintEndPoint2["peerConnectionSuccess"] = "peer_connection_success";
  OctoPrintEndPoint2["peerConnectionError"] = "peer_connection_error";
  OctoPrintEndPoint2["initiateBedLevelling"] = "initiate_bed_levelling";
  OctoPrintEndPoint2["saveMasterPeerID"] = "save_master_peer_id";
  OctoPrintEndPoint2["getBedLevelling"] = "get_bed_levelling";
  return OctoPrintEndPoint2;
})(OctoPrintEndPoint || {});
async function callOctoprintEndPoint(endpointToCall, method = "GET", args) {
  const API_BASEURL = BASEURL + "plugin/NanoFactory/";
  let argsString = "?";
  if (args) {
  ...
  return await fetch(API_BASEURL + endpointToCall + (argsString.length > 1 ? argsString : ""), {
    method,
    headers: {
...
function extractGroupID(peerID) {
  if (peerID.indexOf("-") > 0) {
    return peerID.substring(peerID.indexOf("-") + 1);
  }
  return "";
}
function extractDeviceID(peerID) {
  if (peerID.indexOf("-") > 0) {
...
function sendData(peerID, data, label) {
  ...
  peerConnection.on("open", function () {
    ...
    console.log("Sent: Label-", ConnectionLabels[label], " Data: ", data, " to peerID: ", peerID);
  });
  peerConnection.on("error", async function (err) {
    console.error("Could not send data: " + JSON.stringify(data), " to peerID: ", peerID, "\n Error: ", err);
...
function sendFile(peerID, fileContent, metadata, label) {
  ...
  peerConnection.on("close", function () {
    console.log(" Sent file. Label:" + ConnectionLabels[label]);
  });
...
async function handleJob(data, peerID, label, metadata, fileContent) {
  switch (label) {
    case ConnectionLabels.jobCreated:
      let job = new PrintJob(metadata["id"], metadata["printerID"], metadata["jobName"], metadata["assignerName"], metadata["estimatedPrintTime"], metadata["estimatedFilamentUsage"], metadata["createdDate"], metadata["queuePosition"], null, metadata["filamentID"]);
      job.add("printQueue");
      await OctoPrint.files.upload("local", new File([new Blob([fileContent], { type: "text/plain;charset=utf-8" })], metadata["jobName"] + ".gcode"));
      if ((await db.printer.toArray())[0].state.status === PrinterStatus.operational) {
        startQueue();
      }
      sendDataToAllAvailablePeers({ "data": metadata }, ConnectionLabels.jobCreated);
      break;
...
function sendCurrentJobUpdates(peerID) {
  const options = {
    label: ConnectionLabels.currentJobUpdatesResponse.toString(),
    metadata: peerID,
    serialization: "json"
  };
...
async function handlePrinter(data, peerID, label, _metadata) {
  switch (label) {
    ...
    case ConnectionLabels.bedLevelingRequest:
      ...
      callOctoprintEndPoint(OctoPrintEndPoint.initiateBedLevelling, "GET");
  ...
  setTimeout(() => {
    let bedlevellingResponsePoll = setInterval(async () => {
      if (numberOfBedlevellingResponseTries < BED_LEVELLING_MAX_TRIES) {
        ...
        let response = (await callOctoprintEndPoint(OctoPrintEndPoint.getBedLevelling, "GET")).json();
        if ("data" in response) {
...
async function updatePrinterStateAndTemperature() {
  const TEMPERAUTRE_HISTORY_LENGTH = 50;
  let response = {};
  try {
    response = await OctoPrint.printer.getFullState({ history: true, limit: TEMPERAUTRE_HISTORY_LENGTH });
  } catch (error) {
    response = {
      "state": {
        "text": "Offline"
...
function startTemperatureStream(peerID) {
  const options = {
    label: ConnectionLabels.temperatureStreamResponse.toString(),
    metadata: peerID,
    serialization: "json"
  };
...
function startTerminalStream(peerID) {
  const options = {
    label: ConnectionLabels.terminalResponse.toString(),
    metadata: peerID,
    serialization: "json"
  };
...
function startPositionChangedStream(peerID) {
  const options = {
    label: ConnectionLabels.positionChangedResponse.toString(),
    metadata: peerID,
    serialization: "json"
  };
...
var socketEventTypes = /* @__PURE__ */ ((socketEventTypes2) => {
  ...
  socketEventTypes2["PLUGIN"] = "plugin";
...
async function handleSyncAllRequest(peerID) {
  ...
  if (networking.masterPeerID.includes("-")) {
    if (peerID.includes("-")) {
      payload.is_master = extractGroupID(networking.masterPeerID) === extractGroupID(peerID);
    }
  } else {
...
async function handleSocketMessage(message) {
  ...
  switch (message.event) {
    ...
    case socketEventTypes.EVENT:
      ...
      break;
    case socketEventTypes.PLUGIN:
      if (Object.keys(message.data.data).includes("deleteDatabase")) {
        await db.printQueue.clear();
        await db.filaments.clear();
        await db.nanofactoryFiles.clear();
        await db.jobsHistory.clear();
...
function callbackFunctionsForPeer() {
  peer.on("open", function (id) {
    ...
    callOctoprintEndPoint(OctoPrintEndPoint.peerConnectionSuccess, "GET");
  });
  peer.on("connection", function (connection) {
    connection.on("data", function (data) {
      let numberLabel = parseInt(connection.label);
      console.log("Received Label: " + ConnectionLabels[numberLabel] + " from peerID: " + connection.peer);
      handleIncomingData(data, connection.peer, numberLabel, connection.metadata);
      if (!CONTINUOUS_CONNECTION_LABELS.includes(connection.label)) {
        connection.close();
      }
    });
  });
  ...
  peer.on("error", async function (err) {
    ...
    if (err.message === 'ID "' + peer.id + '" is taken') {
      console.log("Will retry connection to Peer server in " + RETRY_CONNECTION_TIMEOUT + " seconds!");
      callOctoprintEndPoint(OctoPrintEndPoint.peerConnectionError, "GET", { "timeout": RETRY_CONNECTION_TIMEOUT });
      setTimeout(() => {
        peer = new $70d766613f57b014$export$2e2bcd8739ae039(networking.peerID);
        callbackFunctionsForPeer();
